...
public class CommonFunc {
	...
	public static String MD5(String input) {
		...
		if (digest != null) {
			digest.reset();
			try {
				return Base64.encodeToString(digest.digest(input.getBytes("utf-8")), Base64.NO_WRAP);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
		}
	...
	public static String decrypt(String sEncypted, String sKey) {
		try {
			...
			byte[] encrypted = Base64.decode(sEncypted, Base64.NO_WRAP);
	...
	public static boolean writeFile(InputStream is, String filePath, String fileName) throws Exception {
		if (is == null) {
			return false;
		}
