
<!DOCTYPE html>
<html lang="ko">
<head>
<title>S-HR MO Link</title>
<meta charset="utf-8">
<meta name="viewport" content="user-scalable=no, initial-scale=1.0, maximum-scale=1.0, width=device-width" id="viewport">
<link href="/resources2/app/images/common/favicon.ico" rel="shrtcut icon">
<link href="/resources2/app/images/common/favicon.png" rel="apple-touch-icon-precomposed">
<link href="/resources2/app/css/base.css" rel="stylesheet" type="text/css">
</head>
<body class="link">
<div class="linkHtml">
	<h1>
		<a href="/resources2/app/html/guide/link.jsp">MO HTML Link</a>
		<!-- <a class="tg" href="/">홈</a>  -->
	</h1>
	<div class="icoPack"><em class="ing">ing</em> <em class="com">com</em></div>
	<section class="cont">
		<ul>
			<li>
				<h3><a href="javascript:;">가이드</a></h3>
				<ul>
					<li>
						<h4>UI가이드</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/guide.jsp</mark>
					</li>
					<li>
						<h4>레이아웃_기본</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/layout_default.jsp</mark>
					</li>
					<li>
						<h4>레이아웃_빈거</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/layout_blank.jsp</mark>
					</li>
					<li>
						<h4>팝(페이지)</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/popPage.jsp</mark>
					</li>
					<li>
						<h4>팝(레이어)</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/popLayer.jsp</mark>
					</li>
				</ul>
			</li>
			<li>
				<h3><a href="javascript:;">공통</a></h3>
				<ul>
					<li>
						<h4>검색결과</h4> <em>ing</em>
						<mark>/resources2/app/html/common/searchResult.jsp</mark>
					</li>
					<li>
						<h4>쿨</h4> <em>ing</em>
						<mark>/resources2/app/html/common/cool.jsp</mark>
					</li>
					<li>
						<h4>댓글</h4> <em>ing</em>
						<mark>/resources2/app/html/common/comments.jsp</mark>
					</li>
					<!-- <li>
						<h4>팝업윈도우</h4> <em>ing</em>
						<mark>/resources2/app/html/guide/popWin.jsp</mark>
					</li> -->
				</ul>
			</li>
			<li>
				<h3><a href="javascript:;">홈</a></h3>
				<ul>
					<li>
						<h4>홈</h4> <em>ing</em>
						<mark>/resources2/app/html/home/home.jsp</mark>
					</li>
					<li>
						<h4>인트로</h4> <em>ing</em>
						<mark>/resources2/app/html/home/intro.jsp</mark>
					</li>
					<li>
						<h4>정보입력</h4> <em>ing</em>
						<mark>/resources2/app/html/home/myinfo_step1.jsp</mark>
					</li>
				</ul>
			</li>
			<li>
				<h3><a href="javascript:;">로그인,가입</a></h3>
				<ul>
					<li>
						<h4>로긴,가입</h4> <em>ing</em>
						<mark>/resources2/app/html/member/joinPre.jsp</mark>
					</li>
					<li>
						<h4>로그인</h4> <em>ing</em>
						<mark>/resources2/app/html/member/login.jsp</mark>
					</li>
					<li>
						<h4>가입</h4> <em>ing</em>
						<mark>/resources2/app/html/member/join.jsp</mark>
					</li>
					<li>
						<h4>탈퇴</h4> <em>ing</em>
						<mark>/resources2/app/html/member/mem_dis.jsp</mark>
					</li>
				</ul>
			</li>
			<li>
				<h3><a href="javascript:;">룩</a></h3>
				<ul>
					<li>
						<h4>리스트</h4> <em>ing</em>
						<mark>/resources2/app/html/look/look.jsp</mark>
					</li>
					<li>
						<h4>상세</h4> <em>ing</em>
						<mark>/resources2/app/html/look/view.jsp</mark>
					</li>
				</ul>
			</li>
			<li>
				<h3><a href="javascript:;">마이페이지</a></h3>
				<ul>
					<li>
						<h4>설정</h4> <em>ing</em>
						<mark>/resources2/app/html/mypage/setting.jsp</mark>
					</li>
					<li>
						<h4>프로필</h4> <em>ing</em>
						<mark>/resources2/app/html/mypage/profile.jsp</mark>
					</li>
					<li>
						<h4>프로필설정</h4> <em>ing</em>
						<mark>/resources2/app/html/mypage/profile_set.jsp</mark>
					</li>
					<li>
						<h4>알림</h4> <em>ing</em>
						<mark>/resources2/app/html/mypage/alim.jsp</mark>
					</li>
					<li>
						<h4>팔로워</h4> <em>ing</em>
						<mark>/resources2/app/html/mypage/follow.jsp</mark>
					</li>
				</ul>
			</li>
		</ul>
	</section>
</div>
<script src="/resources2/app/js/jquery-3.4.1.js"></script>
<script src="/resources2/app/html/guide/html.js"></script>
<script>
	uiHtml.menu.linkStat();
</script>
</body>
</html>
